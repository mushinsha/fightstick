...
Kaimana::Kaimana(void)
{
  ...
  pinMode( PIN_DOWN,   INPUT_PULLUP );    // DPAD DOWN
  pinMode( PIN_UP,     INPUT_PULLUP );    // DPAD UP
  pinMode( PIN_LEFT,   INPUT_PULLUP );    // DPAD LEFT
  pinMode( PIN_RIGHT,  INPUT_PULLUP );    // DPAD RIGHT
  pinMode( PIN_HOME,   INPUT_PULLUP );    // HOME = GUIDE
  pinMode( PIN_SELECT, INPUT_PULLUP );    // SELECT = BACK 
  pinMode( PIN_START,  INPUT_PULLUP );    // START = OPTIONS
  pinMode( PIN_P1,     INPUT_PULLUP );    // SQUARE
  pinMode( PIN_P2,     INPUT_PULLUP );    // TRIANGLE
  pinMode( PIN_P3,     INPUT_PULLUP );    // RIGHT BUMPER
  pinMode( PIN_P4,     INPUT_PULLUP );    // LEFT BUMPER
  pinMode( PIN_K1,     INPUT_PULLUP );    // CROSS
  pinMode( PIN_K2,     INPUT_PULLUP );    // CIRCLE
  pinMode( PIN_K3,     INPUT_PULLUP );    // RIGHT TRIGGER
  pinMode( PIN_K4,     INPUT_PULLUP );    // LEFT TRIGGER
